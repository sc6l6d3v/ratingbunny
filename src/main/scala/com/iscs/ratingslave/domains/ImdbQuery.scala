...
class ImdbQueryImpl[F[_]: MonadCancelThrow: Async: Parallel: Concurrent](
                                                                         ...
                                                                         imageHost: String,
                                                                         client: Client[F]) extends ImdbQuery[F]
  with DecodeUtils with QuerySetup {
   ...
   * },
   * { $sort: {
   *     startYear: -1, numVotes: -1, averageRating: -1,  primaryTitle: 1 }
   *     }
   * },
   * { $limit: 120 }
   * ])
  ...
  private def getPath(imdb: String): F[PathRec] = {
    ...
    for {
      result <- Clock[F].timed(client.expect(pathReq)(jsonOf[F, PathRec]))
